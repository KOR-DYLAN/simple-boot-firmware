...
def config_text(symbols, values, names=None, heading=None):
    ...
    for name in sorted(names if names is not None else symbols):
...
def main():
    ...
    try:
        ...
        if args.save_defconfig:
            ...
            names = changed_names(symbols, values, defaults)
            atomic_write(args.save_defconfig, config_text(
        ...
        if args.save_fragment:
            ...
            atomic_write(args.save_fragment, config_text(
                symbols, values, names,
                "# Configuration fragment relative to the selected board configs."
            ))
